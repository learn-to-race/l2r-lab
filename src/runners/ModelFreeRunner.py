...
class ModelFreeRunner(BaseRunner):
    ...
    def __init__(
        ...
        self.model_update_iter = model_update_iter
        ...
        self.configs = {
            "agent": get_configurable_dict(agent_config_path, NameToSourcePath.agent),
            "encoder": get_configurable_dict(
                encoder_config_path, NameToSourcePath.encoder
            ),
            "buffer": get_configurable_dict(
                buffer_config_path, NameToSourcePath.buffer
            ),
        ...
        for ep_number in range(start_idx + 1, self.num_run_episodes + 1):
            ...
            while not done:
                ...
                if (t >= self.update_model_after) and (
                    ...
                    for _ in range(self.model_update_iter):

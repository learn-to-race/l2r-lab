...
class Inference():
    ...
    def run_for_img(self, input_img):
        self.model.eval()
        with torch.no_grad():
            # Need to convert input_img to PIL format for compatibility
            input_img = Image.fromarray(input_img)

            inputs_val = {'idx': torch.tensor([0]),
                          'filename': [''],
                          'is_labeled': torch.tensor([True]),
                          ('color',0,-1): input_img}
            self.val_loader.preprocess(inputs_val, False)
            for k, v in inputs_val.items():
                if torch.is_tensor(v):
                    inputs_val[k] = v.to(self.device, non_blocking=True)

            inputs_val[("color", 0, 0)] = torch.unsqueeze(inputs_val[("color", 0, 0)], dim = 0)
            inputs_val[("color_aug", 0, 0)] = torch.unsqueeze(inputs_val[("color_aug", 0, 0)], dim = 0)
